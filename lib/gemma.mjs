...
// Free-form study coach for the Learn hub's "Ask Gemma" box. Unlike
// generateAnswerWithLocalGemma (which writes first-person job-application
// answers), this answers as an interview coach about whatever prep page the
// candidate is reading, grounded in their real background when relevant.
async function askLearnTutorWithLocalGemma(input) {
  ...
  const prompt = `You are an expert technical interview coach helping a senior software engineer prepare for interviews. Be accurate, specific, and concise.
